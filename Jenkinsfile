        ...
        stage ('Sonar Analysis') {
            ...
            steps {
                ...
                withCredentials([string(credentialsId: 'sQ-Tbend', variable: 'sonarLogin')]){
                    sh "${scannerHome}/bin/sonar-scanner -e -Dsonar.projectKey=PipelineBackend -Dsonar.host.url=http://acer:9000  -Dsonar.login=${sQ-Tbend} -Dsonar.java.binaries=target -Dsonar.coverage.exclusions=**/mvn/**,**/src/test/**,**/model/**,**Application.java"
                }
            }
        }
        stage ('Quality Gate') {
